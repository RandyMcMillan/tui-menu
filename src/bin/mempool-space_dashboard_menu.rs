            ...
            for e in self.menu.drain_events() {
                match e {
                    MenuEvent::Selected(item) => match item {
                        Action::Mainnet => {
                            self.content = String::from("Mainnet");
                            //return Ok(());
                        }
                        Action::Signet => {
                            self.content = String::from("Signet");
                            //return Ok(());
                        }
                        Action::Testnet3 => {
                            self.content = String::from("Testnet3");
                            //return Ok(());
                        }
                        Action::Testnet4 => {
                            self.content = String::from("Testnet4");
                            //return Ok(());
                        }
                        Action::Exit => {
                            return Ok(());
...
impl Widget for &mut App {
    fn render(self, area: Rect, buf: &mut Buffer) {
        ...
        Paragraph::new(self.content.as_str())
            .block(Block::bordered().title("─ ─────**───  ──").on_black())
            .render(main, buf);

        let [log, menu] = Layout::horizontal([Length(17), Fill(1)]).areas(area);
        "  mempool-space  "
            .bold()
            .blue()
            .into_centered_line()
